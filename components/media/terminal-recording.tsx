'use client'

/**
 * TerminalRecording — MP4/WebM screen recording with macOS-style chrome,
 * chapter navigation, transcript toggle, and VideoObject JSON-LD.
 *
 * Usage in MDX:
 *   <TerminalRecording
 *     src="/recordings/session-01.mp4"
 *     title="Building the content pipeline"
 *     duration={342}
 *     uploadDate="2026-05-17"
 *     chapters={[
 *       { title: "Project setup", start: 0 },
 *       { title: "First file", start: 45 },
 *     ]}
 *     transcript={[
 *       { time: 0, speaker: "Dev", text: "Starting fresh Next.js project..." },
 *     ]}
 *   />
 */

import { useRef, useState } from 'react'
import type { MediaChapter, TranscriptEntry } from '@/lib/media'
import { formatTimestamp, formatISO8601Duration } from '@/lib/media'
import { TranscriptBlock } from './transcript-block'

interface TerminalRecordingProps {
  src: string
  title: string
  duration: number
  uploadDate: string
  description?: string
  thumbnailUrl?: string
  chapters?: MediaChapter[]
  transcript?: TranscriptEntry[]
  /** Poster image shown before play */
  poster?: string
}

export function TerminalRecording({
  src,
  title,
  duration,
  uploadDate,
  description,
  thumbnailUrl,
  chapters = [],
  transcript = [],
  poster,
}: TerminalRecordingProps) {
  const videoRef = useRef<HTMLVideoElement>(null)
  const [activeChapter, setActiveChapter] = useState(0)
  const [showTranscript, setShowTranscript] = useState(false)

  function seekTo(seconds: number, index: number) {
    if (videoRef.current) {
      videoRef.current.currentTime = seconds
      videoRef.current.play().catch(() => {})
    }
    setActiveChapter(index)
  }

  // VideoObject JSON-LD
  const jsonLd = {
    '@context': 'https://schema.org',
    '@type': 'VideoObject',
    name: title,
    description: description ?? title,
    duration: formatISO8601Duration(duration),
    uploadDate,
    ...(thumbnailUrl ? { thumbnailUrl } : {}),
    ...(chapters.length > 0 ? {
      hasPart: chapters.map((ch, i) => ({
        '@type': 'Clip',
        name: ch.title,
        startOffset: ch.start,
        endOffset: ch.end ?? (chapters[i + 1]?.start ?? duration),
        url: `#${ch.anchor ?? ''}`,
      })),
    } : {}),
  }

  return (
    <figure className="my-8 rounded-xl overflow-hidden border border-surface-700/60 shadow-xl bg-surface-950">

      {/* JSON-LD */}
      <script
        type="application/ld+json"
        dangerouslySetInnerHTML={{ __html: JSON.stringify(jsonLd) }}
      />

      {/* macOS-style titlebar */}
      <div className="flex items-center gap-2 px-4 py-2.5 bg-surface-900/80 border-b border-surface-800/60">
        <span className="w-3 h-3 rounded-full bg-red-500/70" />
        <span className="w-3 h-3 rounded-full bg-yellow-500/70" />
        <span className="w-3 h-3 rounded-full bg-green-500/70" />
        <span className="mx-auto text-xs text-surface-500 font-mono truncate px-4">{title}</span>
        <span className="text-[10px] font-mono text-surface-700">{formatTimestamp(duration)}</span>
      </div>

      {/* Video */}
      <div className="aspect-video bg-black">
        {/* eslint-disable-next-line jsx-a11y/media-has-caption */}
        <video
          ref={videoRef}
          src={src}
          poster={poster}
          controls
          preload="metadata"
          className="w-full h-full object-contain"
          aria-label={title}
        />
      </div>

      {/* Chapter strip */}
      {chapters.length > 0 && (
        <div className="border-t border-surface-800/60 bg-surface-900/60">
          <div className="flex items-center gap-1 px-3 py-2 overflow-x-auto scrollbar-none">
            {chapters.map((ch, i) => (
              <button
                key={i}
                onClick={() => seekTo(ch.start, i)}
                className={[
                  'shrink-0 flex items-center gap-1.5 rounded-md px-2.5 py-1 text-[11px] font-mono transition-colors',
                  activeChapter === i
                    ? 'bg-brand-500/20 text-brand-300 border border-brand-500/30'
                    : 'text-surface-500 hover:text-surface-300 hover:bg-surface-800/60 border border-transparent',
                ].join(' ')}
              >
                <span className="text-surface-600">{formatTimestamp(ch.start)}</span>
                <span className="truncate max-w-[120px]">{ch.title}</span>
              </button>
            ))}
          </div>
        </div>
      )}

      {/* Caption + transcript toggle */}
      <figcaption className="flex items-center justify-between px-4 py-2 bg-surface-900/40 border-t border-surface-800/40">
        <span className="text-xs text-surface-500 font-mono">{title}</span>
        {transcript.length > 0 && (
          <button
            onClick={() => setShowTranscript(v => !v)}
            className="text-[11px] font-mono text-surface-600 hover:text-surface-400 transition-colors"
          >
            {showTranscript ? 'Hide transcript' : 'Show transcript'}
          </button>
        )}
      </figcaption>

      {/* Transcript */}
      {showTranscript && transcript.length > 0 && (
        <div className="border-t border-surface-800/40">
          <TranscriptBlock entries={transcript} defaultOpen />
        </div>
      )}
    </figure>
  )
}
