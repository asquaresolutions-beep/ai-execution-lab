...
<Callout type="info" title="Local browser success is not propagation verification">
...
3. For GitHub Pages deployments: confirm the "Enforce HTTPS" checkbox is available (not greyed out) in the repository Pages settings before announcing the domain. This is the terminal indicator that both DNS and certificate provisioning are complete.
4. Set DNS record TTL to 300 seconds (5 minutes) before any planned DNS change — this reduces the propagation window for subsequent changes.
5. Add DNS propagation verification as a required step in any deployment checklist that involves a new custom domain.
...
| Dependency Default Behavioral Changes | Library upgrade ships breaking behavior as opt-out default | Silent (build passes, render fails) | [next-mdx-remote-v6-blockjs](/failures/next-mdx-remote-v6-blockjs) |
| Runtime Environment Scope Drift | Dev/prod environment divergence for runtime or config | Delayed (works locally, fails in production) | [edge-runtime-deployment-failure](/failures/edge-runtime-deployment-failure), [environment-variable-missing-production](/failures/environment-variable-missing-production) |
| Infrastructure Timing Dependencies | External propagation with non-zero TTL tested too early | Delayed and inconsistent (works for developer, fails for users) | [dns-subdomain-propagation-delay](/failures/dns-subdomain-propagation-delay) |
