// ─────────────────────────────────────────────────────────────────
// lib/trustseal/messages/hi.ts  (asq-trustseal-harden)
// Hindi (हिन्दी) catalogue. Machine-translated (best-effort) — recommend a native
// review of legal/marketing copy. Shape matches Messages (tsc-enforced). Brand
// names and prices are left as-is.
// ─────────────────────────────────────────────────────────────────
import type { Messages } from './en'

export const hi: Messages = {
  common: {
    product: 'TrustSeal',
    company: 'A Square Solutions',
    tagline: 'व्यापार में भरोसा, प्रतिष्ठा और सत्यापन',
    getStarted: 'शुरू करें',
    talkToSales: 'सेल्स से बात करें',
    verifyNow: 'डोमेन सत्यापित करें',
    learnMore: 'और जानें',
    copyright: '© 2026 A Square Solutions. सर्वाधिकार सुरक्षित।',
  },
  nav: {
    product: 'उत्पाद',
    verify: 'सत्यापित करें',
    pricing: 'मूल्य',
    enterprise: 'एंटरप्राइज़',
    customers: 'ग्राहक',
    security: 'सुरक्षा',
    docs: 'दस्तावेज़',
    about: 'परिचय',
    dashboard: 'डैशबोर्ड',
    signIn: 'साइन इन',
    signOut: 'साइन आउट',
    menu: 'मेन्यू',
    trustCenter: 'ट्रस्ट सेंटर',
  },
  switcher: { label: 'भाषा' },
  dash: {
    kicker: 'TrustSeal · डैशबोर्ड',
    title: 'डैशबोर्ड',
    loading: 'लोड हो रहा है…',
    notConfigured: 'इस वातावरण के लिए साइन-इन कॉन्फ़िगर नहीं है।',
    signInTitle: 'अपने डैशबोर्ड में साइन इन करें',
    signInBody: 'अपने सत्यापित डोमेन, बैज और बिलिंग प्रबंधित करें।',
    signedInAs: 'इस रूप में साइन इन',
    accountError: 'खाता लोड नहीं हो सका',
    greeting: 'नमस्ते',
    domainsTitle: 'आपके डोमेन',
    domainsEmpty: 'अभी कोई डोमेन नहीं। शुरू करने के लिए ऊपर एक सत्यापित करें।',
    domainsError: 'डोमेन लोड नहीं हो सके',
    added: 'जोड़ा गया',
    verifiedOn: 'सत्यापित',
    remove: 'हटाएँ',
    removing: 'हटाया जा रहा है…',
    removeConfirm: '{domain} के लिए लंबित दावा हटाएँ? इसे पूर्ववत नहीं किया जा सकता।',
    removeFailed: 'हटाना विफल',
    statusPending: 'लंबित',
    statusVerified: 'सत्यापित',
    statusFailed: 'विफल',
    statusRevoked: 'रद्द',
    verifyTitle: 'डोमेन सत्यापित करें',
    verifyBody: 'DNS TXT रिकॉर्ड जोड़कर साबित करें कि डोमेन आपके नियंत्रण में है।',
    domainPlaceholder: 'example.com',
    startClaim: 'दावा शुरू करें',
    starting: 'शुरू हो रहा है…',
    pending: 'लंबित',
    verifying: 'सत्यापित हो रहा है…',
    txtInstruction: 'अपने DNS प्रदाता पर यह TXT रिकॉर्ड जोड़ें, फिर सत्यापित करें पर क्लिक करें।',
    forDomain: 'के लिए',
    nameHost: 'नाम / होस्ट',
    value: 'मान',
    fallbackHint: 'एपेक्स रिकॉर्ड संपादित नहीं कर सकते? इसके बजाय इसे यहाँ जोड़ें',
    verifyOwnership: 'स्वामित्व सत्यापित करें',
    checkStatus: 'स्थिति जाँचें',
    cancel: 'रद्द करें',
    copy: 'कॉपी करें',
    copied: 'कॉपी हो गया ✓',
    verifiedSuffix: 'सत्यापित',
    verifiedBody: 'अब यह डोमेन TrustSeal में आपका है। यह नीचे आपके डोमेन में दिखता है।',
    verifyAnother: 'दूसरा डोमेन सत्यापित करें',
    errInvalidDomain: 'यह मान्य डोमेन नहीं लगता। example.com जैसा डोमेन दर्ज करें।',
    errAlreadyClaimed: 'यह डोमेन पहले से किसी अन्य खाते द्वारा सत्यापित है।',
    errTxtNotFound: 'TXT रिकॉर्ड अभी नहीं मिला। DNS को फैलने में कुछ मिनट लग सकते हैं — रिकॉर्ड जोड़ें और पुनः प्रयास करें।',
    errExpired: 'यह दावा समाप्त हो गया है। ताज़ा रिकॉर्ड के लिए नया दावा शुरू करें।',
    errRateLimited: 'बहुत अधिक प्रयास। कृपया एक मिनट प्रतीक्षा करें और पुनः प्रयास करें।',
    errEnterDomain: 'जारी रखने के लिए डोमेन दर्ज करें।',
    errUnauthorized: 'आपका सत्र समाप्त हो गया। पेज रिफ़्रेश करें और फिर साइन इन करें।',
    errGeneric: 'कुछ गलत हो गया। कृपया पुनः प्रयास करें।',
    billingTitle: 'बिलिंग',
    proBadge: 'PRO',
    freeBadge: 'FREE',
    planLabel: 'योजना',
    statusLabel: 'स्थिति',
    renewsLabel: 'नवीनीकरण / समाप्ति',
    accessUntilLabel: 'पहुँच इस तिथि तक',
    historyLabel: 'बिलिंग इतिहास',
    invoicesSoon: 'चालान जल्द आ रहे हैं',
    planFree: 'मुफ़्त',
    cancelledActive: 'रद्द (अवधि के अंत तक सक्रिय)',
    graceSuffix: ' (छूट अवधि)',
    cancelledBanner: 'सदस्यता रद्द — पहुँच {date} तक बनी रहेगी',
    upgradeMonthly: 'अपग्रेड · मासिक (₹499)',
    upgradeYearly: 'अपग्रेड · वार्षिक (₹4,990)',
    businessBadge: 'BUSINESS',
    upgradeBusiness: 'बिज़नेस लें · ₹1,999/माह',
    proToBusinessTitle: 'निरंतर निगरानी चाहिए?',
    proToBusinessBody: 'बिज़नेस में निगरानी + अलर्ट, ट्रस्ट इतिहास, प्रमाणपत्र और 5× API कोटा (3,000 बनाम 600 अनुरोध/मिनट) मिलता है।',
    businessContact: 'बिज़नेस चेकआउट सक्षम किया जा रहा है — आज अपग्रेड के लिए सेल्स से संपर्क करें।',
    cancelSub: 'सदस्यता रद्द करें',
    reactivate: 'सदस्यता पुनः सक्रिय करें',
    billingError: 'बिलिंग लोड नहीं हो सकी',
    apiTitle: 'API एक्सेस',
    apiKey: 'आपकी API कुंजी',
    apiPlan: 'योजना',
    apiPerMinute: 'अनुरोध / मिनट',
    apiMonthly: 'मासिक सीमा',
    apiUsageThisMonth: 'इस माह उपयोग',
    apiNotConfigured: 'इस वातावरण में API कुंजियाँ सक्षम नहीं हैं।',
    apiHint: 'हेडर x-api-key या ?key= के रूप में भेजें — दस्तावेज़ देखें।',
    apiRotate: 'पुनः बनाएँ',
    apiRevoke: 'रद्द करें',
    apiRevoked: 'कुंजी रद्द। नई पाने के लिए पुनः बनाएँ।',
    apiRotateConfirm: 'अपनी API कुंजी पुनः बनाएँ? वर्तमान कुंजी तुरंत काम करना बंद कर देगी।',
    apiRevokeConfirm: 'अपनी API कुंजी रद्द करें? मौजूदा एकीकरण काम करना बंद कर देंगे।',
  },
  timeline: {
    heading: 'ट्रस्ट इतिहास',
    verified: 'डोमेन स्वामित्व सत्यापित',
    reverified: 'पुनः-सत्यापित',
    band: 'ट्रस्ट स्तर बदला',
    score: 'ट्रस्ट स्कोर बदला',
    ssl: 'SSL स्थिति बदली',
    dns: 'DNS रिकॉर्ड बदले',
    empty: 'अभी कोई अतिरिक्त इतिहास नहीं।',
  },

  cert: {
    title: 'सत्यापन प्रमाणपत्र',
    subtitle: 'TrustSeal द्वारा डोमेन सत्यापन का आधिकारिक प्रमाण।',
    certificateId: 'प्रमाणपत्र आईडी',
    domainLabel: 'डोमेन',
    trustLevel: 'ट्रस्ट स्तर',
    trustScore: 'ट्रस्ट स्कोर',
    verifiedOn: 'सत्यापन तिथि',
    reverifyDue: 'पुनः-सत्यापन देय',
    verificationUrl: 'सत्यापन URL',
    fingerprint: 'प्रमाणपत्र फिंगरप्रिंट (SHA-256)',
    signature: 'हस्ताक्षर',
    issuedBy: 'जारीकर्ता',
    issuedByBody: 'TrustSeal द्वारा जारी, A Square Solutions का उत्पाद। प्रामाणिकता सत्यापन URL पर या प्रमाणपत्र फ़ील्ड पर फिंगरप्रिंट पुनः-गणना करके सत्यापित की जा सकती है।',
    scanToVerify: 'सत्यापित करने के लिए स्कैन करें',
    download: 'डाउनलोड / प्रिंट',
    downloadCertificate: 'प्रमाणपत्र डाउनलोड करें',
  },

  seal: {
    ownershipVerified: 'डोमेन स्वामित्व TrustSeal द्वारा सत्यापित',
    status: 'TrustSeal स्थिति',
    method: 'सत्यापन विधि',
    verifiedOn: 'सत्यापित दिनांक',
    lastChecked: 'अंतिम जाँच',
    reverification: 'पुनः-सत्यापन',
    ownershipVerifiedShort: 'स्वामित्व सत्यापित',
    trustSummary: 'ट्रस्ट सारांश',
    trustScore: 'ट्रस्ट स्कोर',
    confidence: 'विश्वास',
    partial: 'आंशिक कवरेज',
    preparing: 'स्वामित्व सत्यापित है। इस डोमेन के लिए पूर्ण ट्रस्ट रिपोर्ट तैयार की जा रही है।',
    publicRecord: 'सार्वजनिक रिकॉर्ड',
    verificationBy: 'सत्यापन द्वारा',
    badgePreview: 'बैज पूर्वावलोकन',
    embedHint: 'अपनी साइट पर लाइव बैज दिखाने के लिए यह टैग जोड़ें:',
    publicSignals: 'सार्वजनिक संकेत',
    scoreBreakdown: 'स्कोर विवरण',
    reasoning: 'यह स्कोर क्यों',
    confidenceLevel: 'विश्वास स्तर',
    coveredYes: 'मूल्यांकित',
    coveredNo: 'मूल्यांकित नहीं',
    trustLevel: 'ट्रस्ट स्तर',
  },

  auth: {
    continueGoogle: 'Google से जारी रखें',
    email: 'ईमेल',
    password: 'पासवर्ड',
    signIn: 'साइन इन',
    createAccount: 'खाता बनाएँ',
    or: 'या',
    switchToSignUp: 'नए हैं? खाता बनाएँ',
    switchToSignIn: 'खाता है? साइन इन करें',
    notConfigured: 'प्रमाणीकरण अभी कॉन्फ़िगर नहीं है।',
    signInFailed: 'साइन-इन विफल',
    googleFailed: 'Google साइन-इन विफल',
    forgotPassword: 'पासवर्ड भूल गए?',
  },

  legal: {
    center: 'कानूनी केंद्र',
    subtitle: 'TrustSeal के लिए नीतियाँ, लाइसेंस और ब्रांड सुरक्षा।',
    lastUpdated: 'अंतिम अद्यतन',
    onThisPage: 'इस पृष्ठ पर',
    policies: 'नीतियाँ',
    contact: 'इस नीति के बारे में प्रश्न? संपर्क करें',
    englishNotice: 'नीति पाठ अंग्रेज़ी में प्रदान किया गया है (नियामक भाषा)।',
    license: 'लाइसेंस',
    trademark: 'ट्रेडमार्क नीति',
    copyright: 'कॉपीराइट',
    security: 'सुरक्षा',
    conduct: 'आचार संहिता',
    privacy: 'गोपनीयता',
    terms: 'शर्तें',
    acceptableUse: 'स्वीकार्य उपयोग',
    dmca: 'DMCA / कॉपीराइट',
  },

  hero: {
    eyebrow: 'ट्रस्ट इंटेलिजेंस प्लेटफ़ॉर्म',
    title: 'किसी भी व्यवसाय को सेकंडों में सत्यापित करें',
    subtitle: 'TrustSeal डोमेन स्वामित्व और प्रतिष्ठा संकेतों को एक सत्यापन-योग्य ट्रस्ट स्कोर में बदल देता है — ताकि ग्राहक, साझेदार और प्लेटफ़ॉर्म जान सकें कि वे किस पर भरोसा कर रहे हैं।',
    ctaPrimary: 'डोमेन सत्यापित करें',
    ctaSecondary: 'मूल्य देखें',
    note: 'सत्यापन के लिए कार्ड की ज़रूरत नहीं · Pro में एम्बेड बैज और कमांड सेंटर मिलता है',
  },
  metrics: {
    heading: 'प्लेटफ़ॉर्म से वास्तविक, लाइव संकेत',
    domainsVerified: 'सत्यापित डोमेन',
    verificationsRun: 'किए गए सत्यापन',
    certificatesIssued: 'जारी प्रमाणपत्र',
    apiRequestsServed: 'API अनुरोध पूर्ण',
    monitoringChecks: 'निगरानी जाँचें',
  },
  how: {
    heading: 'सत्यापन कैसे काम करता है',
    subheading: 'अज्ञात से सत्यापित तक तीन चरण।',
    step1Title: 'अपना डोमेन दावा करें',
    step1Body: 'यह साबित करने के लिए एक DNS TXT रिकॉर्ड जोड़ें कि डोमेन आपके नियंत्रण में है। सबसे पहले सत्यापित करने वाला खाता ही मालिक होता है।',
    step2Title: 'हम ट्रस्ट की गणना करते हैं',
    step2Body: 'TrustSeal स्वामित्व, TLS, प्रतिष्ठा और जोखिम संकेतों का मूल्यांकन कर एक स्पष्ट ट्रस्ट स्कोर बनाता है।',
    step3Title: 'सील दिखाएँ',
    step3Body: 'एक लाइव, छेड़छाड़-रोधी बैज और एक सार्वजनिक सील पेज प्रकाशित करें जो आपकी स्थिति के साथ अपडेट होता है।',
  },
  levels: {
    heading: 'ट्रस्ट स्तर',
    subheading: 'हर डोमेन एक स्पष्ट बैंड से मेल खाता है — कमांड सेंटर सिग्नल सिस्टम के अनुरूप।',
    verifiedName: 'सत्यापित',
    verifiedDesc: 'स्वामित्व पुष्ट और संकेत मजबूत। पूर्ण भरोसा।',
    establishedName: 'स्थापित',
    establishedDesc: 'ठोस और सुसंगत प्रतिष्ठा इतिहास के साथ सत्यापित।',
    limitedName: 'सीमित',
    limitedDesc: 'सत्यापित, पर कम या नए संकेत। सामान्य सावधानी बरतें।',
    cautionName: 'सावधानी',
    cautionDesc: 'विसंगतियाँ पाई गईं। लेन-देन से पहले समीक्षा करें।',
    riskName: 'जोखिम',
    riskDesc: 'ब्लॉकलिस्ट या प्रतिरूपण संकेत। उच्च जोखिम — बचें।',
  },
  feed: {
    heading: 'हाल के सत्यापन',
    subheading: 'ट्रस्ट इंटेलिजेंस की निरंतर धारा।',
    streaming: 'लाइव',
  },
  network: {
    heading: 'आपका कमांड सेंटर',
    subheading: 'आपके सत्यापित हर काम के लिए एक ही डैशबोर्ड: हर डोमेन, उसकी लाइव ट्रस्ट स्थिति, आपका बैज एम्बेड कोड और मॉनिटरिंग अलर्ट — सब एक जगह।',
    cta: 'कमांड सेंटर देखें',
  },
  pricing: {
    heading: 'सरल, पारदर्शी मूल्य',
    subheading: 'मुफ़्त शुरू करें। बैज और इंटेलिजेंस की ज़रूरत हो तो अपग्रेड करें।',
    freeName: 'मुफ़्त',
    freePrice: '₹0',
    freeTagline: 'सत्यापित होने के लिए',
    freeF1: '1 सत्यापित डोमेन',
    freeF2: 'सार्वजनिक सील पेज',
    freeF3: 'मानक पुनः-सत्यापन',
    freeCta: 'शुरू करें',
    proName: 'Pro',
    proPriceMonthly: '₹499/माह',
    proPriceYearly: '₹4,990/वर्ष',
    proTagline: 'भरोसा दिखाने वाले व्यवसायों के लिए',
    proF1: '10 डोमेन तक',
    proF2: 'एम्बेड करने योग्य हस्ताक्षरित बैज',
    proF3: 'ट्रस्ट इंटेलिजेंस कमांड सेंटर',
    proF4: 'उन्नत एनालिटिक्स और प्राथमिकता पुनः-सत्यापन',
    proCta: 'Pro लें',
    yearlyBadge: '2 महीने मुफ़्त',
    gstNote: 'मूल्य में 18% GST शामिल है।',
  },
  faq: {
    heading: 'अक्सर पूछे जाने वाले प्रश्न',
    q1: 'TrustSeal डोमेन को कैसे सत्यापित करता है?',
    a1: 'आप हमारे द्वारा बनाया गया एक DNS TXT रिकॉर्ड जोड़ते हैं। पता चलते ही स्वामित्व की पुष्टि हो जाती है और पहले सत्यापित करने वाला खाता मालिक बनता है।',
    q2: 'क्या बैज छेड़छाड़-रोधी है?',
    a2: 'हाँ। एम्बेड बैज हर लोड पर लाइव स्थिति जाँचता है और दावा किए गए डोमेन से बंधा होता है, इसलिए कॉपी किया गया बैज सत्यापन का दिखावा नहीं कर सकता।',
    q3: 'Pro और मुफ़्त में क्या-क्या है?',
    a3: 'मुफ़्त में एक सत्यापित डोमेन और सार्वजनिक सील पेज मिलता है। Pro में एम्बेड बैज, कमांड सेंटर, एनालिटिक्स और कई डोमेन जुड़ते हैं।',
    q4: 'क्या मैं कभी भी रद्द कर सकता हूँ?',
    a4: 'हाँ। रद्द करने से नवीनीकरण रुक जाता है; आपकी Pro पहुँच उस अवधि के अंत तक बनी रहती है जिसका भुगतान आपने पहले ही कर दिया है।',
    q5: 'क्या आप अंतरराष्ट्रीय व्यवसायों का समर्थन करते हैं?',
    a5: 'TrustSeal अंग्रेज़ी, हिन्दी, स्पेनिश और अरबी में उपलब्ध है, अरबी के लिए पूर्ण दाएँ-से-बाएँ समर्थन के साथ।',
  },
  cta: {
    heading: 'दुनिया को दिखाएँ कि आप पर भरोसा किया जा सकता है',
    subheading: 'आज ही अपना डोमेन सत्यापित करें और एक ऐसा ट्रस्ट संकेत प्रकाशित करें जिसे ग्राहक पहचानते हैं।',
    primary: 'डोमेन सत्यापित करें',
    secondary: 'सेल्स से बात करें',
  },
  verify: {
    title: 'किसी व्यवसाय को सत्यापित करें',
    subtitle: 'किसी भी डोमेन की सार्वजनिक ट्रस्ट स्थिति जाँचें, या अपना डोमेन दावा कर सत्यापित करें।',
    lookupLabel: 'जाँचने के लिए डोमेन दर्ज करें',
    lookupPlaceholder: 'example.com',
    lookupCta: 'ट्रस्ट स्थिति जाँचें',
    lookupHint: 'उस डोमेन का सार्वजनिक सील पेज खोलता है।',
    ownTitle: 'अपना डोमेन सत्यापित करें',
    ownBody: 'साइन इन करें, एक DNS TXT रिकॉर्ड जोड़ें, और मिनटों में अपनी सत्यापित सील प्रकाशित करें।',
    ownCta: 'अपने डैशबोर्ड पर जाएँ',
    invalidDomain: 'मान्य डोमेन दर्ज करें, उदा. example.com',
  },
  pricingPage: {
    title: 'मूल्य',
    subtitle: 'मुफ़्त शुरू करें, और बैज व इंटेलिजेंस की ज़रूरत हो तो अपग्रेड करें। मूल्य में 18% GST शामिल है।',
    compareHeading: 'योजनाओं की तुलना करें',
    featureCol: 'सुविधा',
    domainsRow: 'सत्यापित डोमेन',
    sealPageRow: 'सार्वजनिक सील पेज',
    badgeRow: 'एम्बेड करने योग्य हस्ताक्षरित बैज',
    commandRow: 'कमांड सेंटर',
    analyticsRow: 'उन्नत एनालिटिक्स',
    supportRow: 'सहायता',
    supportFree: 'समुदाय',
    supportPro: 'प्राथमिकता',
    yes: 'हाँ',
    no: '—',
  },
  footer: {
    tagline: 'व्यापार में भरोसा, प्रतिष्ठा और सत्यापन।',
    product: 'उत्पाद',
    company: 'कंपनी',
    legal: 'कानूनी',
    privacy: 'गोपनीयता',
    terms: 'शर्तें',
    security: 'सुरक्षा',
    about: 'परिचय',
    contact: 'संपर्क',
    builtBy: 'TrustSeal को A Square Solutions ने बनाया है।',
  },
}
