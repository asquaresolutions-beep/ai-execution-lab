...
import { loadGraph, loadCapabilities, validateGraph, depth, unlocked, nextProofTasks, capabilityCoverage } from './engine/graph.mjs'
...
import { LAB_ROOT, corpus } from './engine/corpus.mjs'

/** Instructional coverage from the explicit lesson `proves` mappings in lib/tracks.ts. */
const coverage = caps => capabilityCoverage(caps, corpus().tracksTs.lessonCapabilities)
...
cmds.status = () => {
  ...
  p(`capabilities taught: ${cov.taught_count} of ${cov.total}   (instruction exists - teaching is not proving)`)
  ...
  rule()
  p('INSTRUCTIONAL COVERAGE     capability -> lessons that teach it')
  p('  Explicit `proves` mappings from lib/tracks.ts. Never inferred from tags.')
  p('')
  for (const c of cov.taught) {
    p(`  TAUGHT    ${c.id.padEnd(28)}${c.competency.padEnd(14)}${c.teaching_assets.join(', ')}`)
  }
  for (const c of cov.untaught) {
    p(`  UNTAUGHT  ${c.id.padEnd(28)}${c.competency}`)
  }
  if (cov.unknown.length) {
    p('')
    p(`  UNKNOWN (${cov.unknown.length}) - a lesson names a capability that does not exist:`)
    for (const u of cov.unknown) p(`    ${u.lessonId} -> ${u.capability}`)
  }
  rule()
...
cmds.gaps = () => {
  ...
  const cov = coverage(loadCapabilities())
  p(`Capabilities with NO teaching asset (${cov.untaught_count}) - no lesson declares \`proves\` for these:`)
  for (const c of cov.untaught) p(`    ${c.id.padEnd(28)} ${c.competency}`)
  if (cov.unknown.length) {
    p('')
    p(`UNKNOWN lesson -> capability references (${cov.unknown.length}) - reported, never guessed:`)
    for (const u of cov.unknown) p(`    ${u.lessonId} -> ${u.capability}`)
  }
  return 0
}
