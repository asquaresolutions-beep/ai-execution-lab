 ...
 *
 * A lab names the incidents it reproduces in `reproduces: [<incident slug>]`.
 * Nothing is inferred: same vendor, same platform, same tags or same subsystem
 * do not make a lab a reproduction of an incident. Where no lab declares the
 * relationship, Practice is absent and reported as absent.
 *
 * Mirrors findPrinciple(), which resolves related_docs by slug. Tag overlap was
 * used here previously and produced four false positives out of five matches.
 */
...
export function buildUnits() {
  ...
  for (const inc of c.failures) {
    ...
    const beats = {
      ...
      practice: practice
        ? { source: practice.doc.path, present: true, via: practice.via }
        : { source: null, present: false, gap: 'no lab declares it reproduces this incident' },
